...
- TENDENCIAS: (TEXTO PREDETERMINADO)
    ...
    <!-- Checkbox “activar” solo para mostrar/ocultar; no aporta narrativa -->
    <label>
    ...
    <div id="h1_children" class="child hidden">
      <label>
        <input type="checkbox" id="h1_op2" onchange="syncLinks(this)" />
        Afebril
      </label>
      <label>
        <input type="checkbox" id="h1_op3" onchange="syncLinks(this)" />
        Hemodinámicamente estable
      </label>
      <label>
        <input type="checkbox" id="h1_op4" onchange="syncLinks(this)" />
        Dolor **/10
      </label>
      <label>
        <input type="checkbox" id="h1_op5" onchange="syncLinks(this)" />
        Estado general ***1.Bueno 2.Regular 3.Malo
      </label>
    </div>
  </div>
...
- OSTEOMUSCULAR: (TEXTO PREDETERMINADO)
    ...
    <div id="h8_children" class="child hidden">
      <!-- Sub-secciones (como en tu ejemplo) -->
      <label>
...
<script>
  // ---- Helpers de UI (mostrar/ocultar hijos) ----
  ...
  // ---- Conexiones (CONECTAR CON...) usando data-link y ids obligatorios h<Y>_op<X> ----
  // (Dejé la lógica lista; si luego quieres links, solo agrega data-link="#hY_opX" al checkbox origen.)
  function syncLinks(cb){
    ...
    // Si el target controla hijos, sincroniza visibilidad.
    toggleChildren(target);
  }

  // ---- Parser para ** (input), *** (dropdown), =>>> <<< (multilínea), y paréntesis CONECTAR CON ----
  function buildOptionUI(){
    ...
    labels.forEach(label => {
      ...
      // Texto crudo del label (sin el checkbox)
      let raw = label.textContent.replace(/\s+/g, " ").trim();

      // No tocar labels “vacíos” o utilitarios (ya marcados como data-skip por el checkbox)
      // Igual pueden tener texto; se mantienen.
      // Quitar solo el texto utilitario para parseo si ya fue transformado previamente:
      if(label.getAttribute("data-built") === "1") return;

      // Multilínea =>>> <<< (no se muestra en label)
      let multiline = null;
      ...
      // Paréntesis con "CONECTAR CON" NO se muestran ni en label ni narrativa
      // (Si usas el formato (CONECTAR CON OPCION X DE HEADER Y), además setea data-link)
      ...
      parens.forEach(p => {
        ...
        if(inside.toUpperCase().includes("CONECTAR CON")){
          ...
          // Intento de extraer (CONECTAR CON OPCION X DE HEADER Y)
      ...
      // Dropdown ***N.OPCION ...
      // Formato exacto: ***1.OPCION 2.OPCION2 3.OPCION3
      let selectEl = null;
      if(raw.includes("***")){
        const parts = raw.split("***");
        const prefix = parts[0];
        const rest = parts.slice(1).join("***"); // por si hubiera más, aunque no debería

        // extraer opciones tipo "1.Texto 2.Texto2 ..."
        const opts = [];
        const re = /(\d+)\.([^]+?)(?=(\s+\d+\.)|$)/g;
        let match;
        ...
        // sufijo = lo que quede después de haber “consumido” las opciones
        // En este formato, normalmente no hay sufijo extra distinto al texto posterior;
        // pero lo manejamos dejando vacío.
        const suffix = "";
        ...
        // opción vacía inicial (por si quieres permitir “sin selección”)
        ...
        // reconstruir contenido visible del label: prefix + select
        // (*** no se muestra)
        label.innerHTML = "";
      ...
      // Si había multiline, la removimos del label (no se muestra)
      if(mMulti){
        // reconstruir label sin la parte multiline
        label.innerHTML = "";
        label.appendChild(cb);
      ...
      // Caso normal: solo limpia paréntesis CONECTAR CON (ya removidos)
      // y deja lo demás tal cual.
      label.innerHTML = "";
      label.appendChild(cb);
      label.appendChild(document.createTextNode(" " + raw));
      label.setAttribute("data-built", "1");
  ...
  function updateNarrativa(){
    ...
    // Headers en orden (1..9)
    const headers = document.querySelectorAll(".opciones");
    headers.forEach(h => {
      const headerIndex = h.getAttribute("data-header-index");
      const titleEl = h.previousElementSibling; // el h2 justo antes
      const titulo = titleEl ? titleEl.textContent : "";

      // Opciones del header (incluye nested)
      const headerChecks = h.querySelectorAll('input[type="checkbox"]');

      // Considerar “marcada” solo si NO es data-skip
      const markedMeaningful = Array.from(headerChecks).some(cb => cb.checked && cb.getAttribute("data-skip") !== "1");

      // DEFAULT (si no hay ninguna opción “real” marcada en ese header)
      if(!markedMeaningful){
        const defEl = h.querySelector("[data-default-text]");
        if(defEl){
          // DEFAULT =>>> <<< debe respetar saltos de línea reales
      ...
      // Si SÍ hay al menos una opción “real” marcada:
      // NO agregar DEFAULT; concatenar normalmente el título del header y las opciones marcadas.
      // Para que el formato quede como tu ejemplo, ponemos el título precedido por "- "
      narrativa += "- " + titulo + " ";

      // procesar opciones marcadas EN ORDEN de aparición en el DOM
      headerChecks.forEach(cb => {
        if(!cb.checked) return;
        if(cb.getAttribute("data-skip") === "1") return;

        // Multilínea por opción =>>> <<<
        const ml = cb.getAttribute("data-multiline");
        if(ml !== null && ml !== undefined){
          narrativa += ml; // tal cual
          // asegura salto de línea luego
          if(!narrativa.endsWith("\n")) narrativa += "\n";
          return;
        }

        // *** dropdown: usa select asociado
        ...
        // Texto normal: usa el texto visible del label (sin checkbox)
        const lab = cb.closest("label");
        if(lab){
          const txt = lab.textContent.replace(/\s+/g, " ").trim();
          // OJO: este txt ya no incluye paréntesis CONECTAR CON (los removimos)
          narrativa += txt;
          narrativa += " ";
        }
      });

      // Cierra el header con salto de línea
